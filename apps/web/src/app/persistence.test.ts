...
describe('persistence', () => {
  ...
  it('stamps the current version', () => {
    ...
    expect(JSON.parse(window.localStorage.getItem(KEY) ?? '{}')).toMatchObject({ version: 1 })
  })
  ...
  // The case this exists for: a returning user whose localStorage was written
  // by a previous version of the app. It must not throw.
  it.each([
    ['not json at all', 'ids: [react]'],
    ['a payload from an older version', JSON.stringify({ version: 0, trackedIds: [] })],
    ['a payload with the wrong shape', JSON.stringify({ version: 1, trackedIds: 'react' })],
